...
<template>
  <div class="space-y-6">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4 p-4 bg-gray-50 rounded-lg">
      ...
      ]" :key="building.type" class="border rounded-lg p-4 bg-white text-center">
        <h3 class="font-medium mb-2">{{ building.label }}</h3>
        <button @click="loadAuditoriums(building.type)"
          class="px-4 py-2 bg-amber-100 hover:bg-amber-400 rounded-md transition-colors" :disabled="isLoading">
          Получить аудитории
        </button>
      </div>
    </div>
    ...
    <!-- Список аудиторий -->
    <div v-if="Object.keys(auditoriumStore.fullSchedule).length > 0"
      class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5 p-4">
      <div v-for="(schedule, auditorium) in auditoriumStore.fullSchedule" :key="auditorium"
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-2 gap-3">
          <div v-for="(lesson, time) in schedule" :key="time" class="border rounded p-3 transition-all hover:shadow-md"
            :class="{
